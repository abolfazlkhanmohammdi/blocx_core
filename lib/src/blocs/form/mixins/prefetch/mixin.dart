...
import 'package:blocx_core/src/blocs/form/mixins/prefetch/events.dart';
...
mixin BlocxFormPrefetchMixin<F extends BlocxBaseFormEntity<F, E>, P, E extends Enum>
  ...
  bool initInfoFetcher() {
    on<BlocxFormEventPrefetchRequiredInfo>(_fetchRequiredInfo);
    return true;
  }
  ...
  Future<void> _fetchRequiredInfo(
    BlocxFormEventPrefetchRequiredInfo event,
    ...
    for (var i = 0; i < results.length; i++) {
      ...
      await onInfoFetched(key, result.data, emit);
    ...
    if (failedKeys.isNotEmpty) {
      handleError(BlocXErrorCode.errorGettingInitialFormData, emit);
    }
  }
