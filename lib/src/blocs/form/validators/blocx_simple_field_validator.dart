...
/// can only produce **one error**. It wraps the result of [validateSimple]
/// into a list so it is compatible with the multi-error API of [BlocxFieldValidator].
///
/// Subclasses **must implement [validateSimple]** to define the actual validation logic.
...
abstract class BlocxSingleErrorFieldValidator<T> extends BlocxFieldValidator<T> {
  ...
  BlocxSingleErrorFieldValidator();

  /// Internal implementation that converts the result of [validateSimple]
  /// into a list of errors.
  ///
  /// Returns a single-element list if [validateSimple] returns a non-null error,
  ...
  List<String> validate(T value) {
    final error = validateSimple(value);
